...
SELECT * FROM form_header;
	...
	/*FORM DATA*/
	<sql:query dataSource="${snapshot}" var="completeform">
SELECT * FROM fill_form_details;
</sql:query>
...
Select fill_form_header.compliteddate, fill_form_header.formid, fill_form_header.userid, form_header.titletext from fill_form_header INNER JOIN form_header ON fill_form_header.formid=form_header.formid;
							...
							<div class="row">
								<div class="col-xl-12 col-lg-12 col-sm-12">
									<table id="form_datatable"
										...
										<tbody>
											<c:forEach var="row" items="${fillfromlist.rows}">
												<tr>
													<td><c:out value="${row.compliteddate}" /></td>
													<td><c:out value="${row.formid}" /></td>
													<td><c:out value="${row.titletext}" /></td>

													<c:set var="username" value="${row.userid}"></c:set>
													<c:if test="${username eq 1}">
														<c:set var="user" value="Admin"></c:set>
													</c:if>

													<c:if test="${username eq 2}">
														<c:set var="user" value="Client"></c:set>
													</c:if>
													...
													<td class="text-center"><span data-toggle="modal"
														data-target="#all_question_preview"><a
															...
															class="previewform" data-placement="bottom" title=""
															data-original-title="Preview" class="text-info fa-size"
															id="${row.formid}"><i class="fa fa-eye"></i></a></span></td>
												</tr>
											</c:forEach>
										</tbody>
									</table>
								...
								<div class="col-xl-4 col-lg-4 col-sm-4 colmspadding">
									<p class="compact mb-1">
										<span class="font-weight-700">Completed Date</span> <span
											class="displayblock font-medium-2" id="compliteddate"></span>
									</p>
								</div>
							</div>
						</div>
	...
	<script src="jspjs/completed_form.js"></script>
